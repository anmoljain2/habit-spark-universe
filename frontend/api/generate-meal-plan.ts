...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const preferencesDetails = [
      ...
      preferences.contexts && Array.isArray(preferences.contexts) && preferences.contexts.length > 0 ? `- Saved meal plan contexts: ${preferences.contexts.map((c: string) => `"${c}"`).join(', ')}` : '',
    ].filter(Boolean).join('\n');
